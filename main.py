            ...
            for line in inFile:
                if line.find("space directions") != -1:
                    try:
                        ...
                        print('"space direction" error, please check nrrd file format')
                elif 'space origin' in line:
                    v=line.split()
                    s=v[2].split(',')
...
def fun_gamma_analysis(dose1,dose2,dosecrit,discrit,cuoff,maxdose,interfra,maxgamma,fraction):
    ...
    gamma_options = {
        'dose_percent_threshold': int(dosecrit),
        'distance_mm_threshold': int(discrit),
...
if __name__ == '__main__':
    ...
    parser.add_argument("-do", "--dosecrit", required=False, nargs='?',
                        help="criteria for dose deviation, default:3 (%)",default=3)
    parser.add_argument("-di", "--discrit", required=False,  nargs='?',
                        help="criteria for distance to agreement, default:3 (mm)", default=3)
    parser.add_argument("-cu", "--cutoff", required=False, nargs='?',
                        help="The percent lower dose cutoff above which gamma will be calculated. Only applied to the reference grid. Default: 10 (%)", default=10)
    ...
    parser.add_argument("-i", "--interfra", required=False, nargs='?',
                        help=" The fraction which gamma distance threshold is divided into for interpolation. Defaults to 5 (0.6mm step for 3mm criteria)", default=10)
    ...
    #fun_1Dgamma()
    fun_gamma_analysis(args.ref,args.comp,args.dosecrit,args.discrit,args.cutoff,args.maxdose,args.interfra,args.maxgamma,args.fraction)
